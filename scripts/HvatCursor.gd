...
func hide_all():
	paw_cursor.visible = false
	net_cursor.visible = false
	sponge_cursor.visible = false
	hvat_cursor.visible = false

func show_cursor(cursor_name: String):
	"""Показывает один курсор, скрывает остальные"""
	hide_all()
	match cursor_name:
		"paw":
			paw_cursor.visible = true
		"net":
			net_cursor.visible = true
		"sponge":
			sponge_cursor.visible = true
		"hvat":
			hvat_cursor.visible = true

func follow_mouse(cursor_name: String):
	"""Двигает указанный курсор за мышью"""
